...
fn test_index_operation() {
    ...
    // Negative index
    let neg_index_op = IndexOperation::new(vec![Box::new(LiteralOperation::new(Value::int(-1)))]);
    ...
    let index_on_obj = IndexOperation::new(vec![Box::new(LiteralOperation::new(Value::int(0)))]);
    assert!(index_on_obj.apply(&obj).is_err());

    let float_val = Value::float(3.14);
    let index_on_float = IndexOperation::new(vec![Box::new(LiteralOperation::new(Value::int(0)))]);
    assert!(index_on_float.apply(&float_val).is_err());

    let bool_val = Value::bool(true);
    let index_on_bool = IndexOperation::new(vec![Box::new(LiteralOperation::new(Value::int(0)))]);
...
fn test_array_construct_operation() {
    ...
    let element_ops = vec![
        Box::new(FieldAccessOperation::new("a".to_string())) as Box<dyn Operation + Send + Sync>,
        Box::new(FieldAccessOperation::new("b".to_string())) as Box<dyn Operation + Send + Sync>,
    ];
...
fn test_sequence_operation() {
    ...
    let expr_ops = vec![
        vec![Box::new(FieldAccessOperation::new("x".to_string()))
            as Box<dyn Operation + Send + Sync>],
        vec![Box::new(LiteralOperation::new(Value::int(42))) as Box<dyn Operation + Send + Sync>],
    ];
...
fn test_assignment_operation() {
    ...
    let value_ops =
        vec![Box::new(LiteralOperation::new(Value::int(10))) as Box<dyn Operation + Send + Sync>];
