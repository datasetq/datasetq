...
pub fn builtin_url_set_port(args: &[Value]) -> Result<Value> {
    ...
    match &args[0] {
                        ...
                        let set_series = series
                            ...
                            .map_err(|e| {
                                dsq_shared::error::operation_error(format!(
                                    "url_set_port() failed to cast series to utf8: {}",
                                    e
                                ))
                            })?
                            .apply(|s| {
        ...
        Value::LazyFrame(lf) => {
            ...
            builtin_url_set_port(&[Value::DataFrame(df), args[1].clone()])
        }
        _ => Err(dsq_shared::error::operation_error(
            "url_set_port() requires string, array, DataFrame, Series, or LazyFrame",
        )),
    }
}
