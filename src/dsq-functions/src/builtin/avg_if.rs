...
mod tests {
    ...
    fn test_avg_if_series() {
        let series = Series::new("test".into().into(), vec![1i64, 2, 3, 4]);
    ...
    fn test_avg_if_series_no_matches() {
        let series = Series::new("test".into().into(), vec![1i64, 2]);
        let mask_series = Series::new("mask".into().into(), vec![false, false]);
        let result = builtin_avg_if(&[Value::Series(series), Value::Series(mask_series)]).unwrap();
        assert_eq!(result, Value::Null);
    }
    ...
    fn test_avg_if_dataframe() {
        let df = DataFrame::new(vec![
            Series::new("a".into().into(), vec![1i64, 2, 3]),
            Series::new("b".into().into(), vec![4i64, 5, 6]),
        ])
        .unwrap();
