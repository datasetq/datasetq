    ...
    match &args[0] {
            ...
            for col_name in df.get_column_names() {
                if let Ok(series) = df.column(col_name) {
                    if series.dtype().is_numeric() || series.dtype() == &DataType::Utf8 {
                        ...
                        let second_series = Series::new(col_name, second_values);
                        new_series.push(second_series);
                    } else {
        ...
        Value::Series(series) => {
            if series.dtype().is_numeric() || series.dtype() == &DataType::Utf8 {
                ...
                Ok(Value::Series(Series::new(series.name(), second_values)))
            } else {
                Ok(Value::Series(series.clone()))
            }
        }
        _ => Err(dsq_shared::error::operation_error(
            "second() requires timestamp, array, DataFrame, or Series",
        )),
    }
}
