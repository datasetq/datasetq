...
pub fn builtin_titlecase(args: &[Value]) -> Result<Value> {
    ...
    match &args[0] {
        ...
        Value::LazyFrame(lf) => {
            ...
            builtin_titlecase(&[Value::DataFrame(df)])
        }
        _ => Ok(args[0].clone()),
    }
}
...
mod tests {
    ...
    fn test_titlecase_series() {
        let series = Series::new(PlSmallStr::from("test"), &["hello", "world", "test"]);
        let result = builtin_titlecase(&[Value::Series(series)]).unwrap();
        if let Value::Series(result_series) = result {
            let expected = Series::new(PlSmallStr::from("test"), &["Hello", "World", "Test"]);
    ...
    fn test_titlecase_dataframe() {
        let names = Series::new(PlSmallStr::from("name"), &["alice", "bob"]);
        let ages = Series::new(PlSmallStr::from("age"), &[25, 30]);
        ...
        if let Value::DataFrame(result_df) = result {
            let expected_names = Series::new(PlSmallStr::from("name"), &["Alice", "Bob"]);
            let expected_ages = Series::new(PlSmallStr::from("age"), &[25, 30]);
            let expected_df =
                DataFrame::new(vec![expected_names.into(), expected_ages.into()]).unwrap();
