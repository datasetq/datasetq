...
pub fn builtin_systime(args: &[Value]) -> Result<Value> {
    ...
    match &args[0] {
        ...
        Value::DataFrame(df) => {
            ...
            let time_series = Series::new("systime", vec![nanos; df.height()]);
        ...
        Value::Series(series) => {
            let time_series = Series::new("systime", vec![nanos; series.len()]);
    ...
    fn test_builtin_systime_with_dataframe() {
        ...
        match result {
            Value::DataFrame(result_df) => {
                assert!(result_df.get_column_names().contains(&"systime"));
